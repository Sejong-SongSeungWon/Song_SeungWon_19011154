...
%% This function makes Perifocal frame to ECI frame.
...
R_inc_angle_1 = [1 0 0;0 cosd(inc_angle) sind(inc_angle);0 -sind(inc_angle) cosd(inc_angle);0 -sind(inc_angle) cos(inc_angle)];
R_RAAN_3 = [cosd(RAAN) sind(RAAN) 0;-sind(RAAN) cosd(RAAN) 0;0 0 1];

rotation_matrix = ((R_arg_prg_3)*(R_inc_angle_1)*(R_RAAN_3));

% For example) R/xyz = PQW2ECI(30, 15, 30) * R/pqw
